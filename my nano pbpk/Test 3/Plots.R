...
exp_fit <- extract(fit)
P_1=mean(exp_fit$theta[,1]) #liver
P_2=mean(exp_fit$theta[,2]) #spleen
P_3=mean(exp_fit$theta[,3]) #kidneys lungs blood
P_4=mean(exp_fit$theta[,4]) #heart uterus
P_5=mean(exp_fit$theta[,5]) #Skeleton Soft tissue 
P_6=mean(exp_fit$theta[,6]) #brain
x_1=mean(exp_fit$theta[,7]) #liver
x_2=mean(exp_fit$theta[,8]) #spleen
x_3=mean(exp_fit$theta[,9]) #kidneys lungs blood
x_4=mean(exp_fit$theta[,10]) #heart uterus
x_5=mean(exp_fit$theta[,11]) #Skeleton Soft tissue
x_6=mean(exp_fit$theta[,12]) #brain
k_de=mean(exp_fit$theta[,13])
Km=mean(exp_fit$theta[,14]) #micro-g/ml 
Pup=mean(exp_fit$theta[,15]) #ml/h/(ml pcs)
CLE_f=mean(exp_fit$theta[,16]) #1/h
CLE_u=mean(exp_fit$theta[,17]) #1/h


params<-create.params(compartments,BW)
params <- c(params, "P_1"=P_1, "P_2"=P_2, "P_3"=P_3, "P_4"=P_4, "P_5"=P_5, "P_6"=P_6,
            "x_1"=x_1, "x_2"=x_2, "x_3"=x_3, "x_4"=x_4, "x_5"=x_5,"x_6"=x_6, 
            "k_de"=k_de, "Km"=Km, "Pup"=Pup, "CLE_f"=CLE_f,  "CLE_u"=CLE_u)
#print(params)
dose <- doses[1]
...
setwd("C:\\Users\\vassi\\Documents\\Diploma Thesis\\My nano PBPK\\Final Model\\Fitting\\Test 3\\result plots")
