...
ode.func <- function(time, inits, params){
  with( as.list(c(inits,params)),{
    ...
    #Adipose 
    Ad_total <- Mcap_ad + Mtis_ad + Mm_ad
    ...
    list(c(dMcap_lu=dMcap_lu, dMcap_spl=dMcap_spl, dMcap_li=dMcap_li, dMcap_ki=dMcap_ki, dMcap_ht=dMcap_ht, dMcap_br=dMcap_br, dMcap_ut=dMcap_ut, dMcap_skel=dMcap_skel, dMcap_ad=dMcap_ad, dMcap_skin=dMcap_skin, dMcap_rob=dMcap_rob,
           ...
           Skel_total=Skel_total, Ad_total=Ad_total, Rob_total=Rob_total, Blood_total=Blood_total)
    
    
  })}

sample_time <- c(0, 10/60, 1, 1*24, 7*24, 28*24, 56*24) #in hours
#sample_time <- seq(0,1000, by=0.1)
return(as.data.frame(ode(times = sample_time, func = ode.func, y = inits, parms = params, method = "bdf")))
}

setwd("C:\\Users\\vassi\\Documents\\Diploma Thesis\\My nano PBPK\\Final Model\\Sensitivity & Identifiability")

solution <- fme.func(params)
#print(solution)
#rowSums(solution[,2:40])



### SENSITIVITY FUNCTIONS ###
sensitivity <- sensFun(func = fme.func, parms=params, senspar = c("P", "x_fast", "x_rest", "x_brain", "k_de", "Km", "Pup", "CLE_f"), 
                       sensvar = c("Lu_total", "Spl_total", "Li_total", "Ki_total", "Ht_total", "Br_total", "Ut_total", 
                                   "Skel_total", "Ad_total", "Rob_total", "Blood_total"))
SI <- summary(sensitivity)
...
### IDENTIFIABLITY ###
ident_test <- collin(sensitivity, maxcomb = 10000)
plot(ident_test, log="y")

###Create excel with results
wb <- createWorkbook("Sensitivity Identifiability results")
sensitivity_frame <- as.data.frame(sensitivity)
Summary_frame <- as.data.frame(SI)
ident_data <- as.data.frame(ident_test)
addWorksheet(wb, "Analytical Sensitivity")
addWorksheet(wb, "Sensitivity Summary")
addWorksheet(wb, "Identifiability")
writeData(wb, sheet = 1, sensitivity_frame)
writeData(wb, sheet = 2, Summary_frame)
writeData(wb, sheet = 3, ident_data)
saveWorkbook(wb, "Sensitivity Identifiability results.xlsx", overwrite = TRUE)
