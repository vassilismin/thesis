functions{
    ...
    real Q_total; real Vven; real Vart; real Vm_ven; real Vm_art;
    ...
    real P; real x_fast; real x_rest; real x_brain; real k_de; real Km; real Pup;
    real CLE_f;
...
///////////params///////////
    ...
    //Lungs
    dMdt[1] = Q_total*C_ven - Q_total*Ccap_lu - PA_lu*Ccap_lu + PA_lu*Ctis_lu/P;
    dMdt[2] = PA_lu*Ccap_lu - PA_lu*Ctis_lu/P - Pup_lu*Ctis_lu + k_de*M[3];
    dMdt[3]   = Pup_lu*Ctis_lu - k_de*M[3];

    //Spleen
    dMdt[4] = Q_spl*C_art - Q_spl*Ccap_spl - PA_spl*Ccap_spl + PA_spl*Ctis_spl/P;
    dMdt[5] = PA_spl*Ccap_spl - PA_spl*Ctis_spl/P - Pup_spl*Ctis_spl + k_de*M[6];
    dMdt[6]   = Pup_spl*Ctis_spl - k_de*M[6];

    //Liver
    dMdt[7] = Q_li*C_art + Q_spl*Ccap_spl - (Q_li+Q_spl)*Ccap_li - PA_li*Ccap_li + PA_li*Ctis_li/P;
    dMdt[8] = PA_li*Ccap_li - PA_li*Ctis_li/P - Pup_li*Ctis_li + k_de*M[9] - CLE_f*M[8];
    dMdt[9]   = Pup_li*Ctis_li - k_de*M[9];
    dMdt[35] = CLE_f*M[8];

    //Kidneys
    dMdt[10] = Q_ki*C_art - Q_ki*Ccap_ki - PA_ki*Ccap_ki + PA_ki*Ctis_ki/P;
    dMdt[11] = PA_ki*Ccap_ki - PA_ki*Ctis_ki/P - Pup_ki*Ctis_ki + k_de*M[12] - CLE_u*M[11];
    dMdt[12]   = Pup_ki*Ctis_ki - k_de*M[12];
    dMdt[36] = CLE_u*M[11];

    //Heart
    dMdt[13] = Q_ht*C_art - Q_ht*Ccap_ht - PA_ht*Ccap_ht + PA_ht*Ctis_ht/P;
    dMdt[14] = PA_ht*Ccap_ht - PA_ht*Ctis_ht/P - Pup_ht*Ctis_ht + k_de*M[15];
    dMdt[15] = Pup_ht*Ctis_ht - k_de*M[15];

    //Brain
    dMdt[16] = Q_br*C_art - Q_br*Ccap_br - PA_br*Ccap_br + PA_br*Ctis_br/P;
    dMdt[17] = PA_br*Ccap_br - PA_br*Ctis_br/P - Pup_br*Ctis_br + k_de*M[18];
    dMdt[18] = Pup_br*Ctis_br - k_de*M[18];

    //Uterus
    dMdt[19] = Q_ut*C_art - Q_ut*Ccap_ut - PA_ut*Ccap_ut + PA_ut*Ctis_ut/P;
    dMdt[20] = PA_ut*Ccap_ut - PA_ut*Ctis_ut/P - Pup_ut*Ctis_ut + k_de*M[21];
    dMdt[21] = Pup_ut*Ctis_ut - k_de*M[21];

    //Skeleton
    dMdt[22] = Q_skel*C_art - Q_skel*Ccap_skel - PA_skel*Ccap_skel + PA_skel*Ctis_skel/P;
    dMdt[23] = PA_skel*Ccap_skel - PA_skel*Ctis_skel/P - Pup_skel*Ctis_skel + k_de*M[24];
    dMdt[24] = Pup_skel*Ctis_skel - k_de*M[24];

    //Soft tissue
    dMdt[25] = Q_st*C_art - Q_st*Ccap_st - PA_st*Ccap_st + PA_st*Ctis_st/P;
    dMdt[26] = PA_st*Ccap_st - PA_st*Ctis_st/P - Pup_st*Ctis_st + k_de*M[27];
    dMdt[27] = Pup_st*Ctis_st - k_de*M[27];

    //Rest of Body
    dMdt[28] = Q_rob*C_art - Q_rob*Ccap_rob - PA_rob*Ccap_rob + PA_rob*Ctis_rob/P;
    dMdt[29] = PA_rob*Ccap_rob - PA_rob*Ctis_rob/P - Pup_rob*Ctis_rob + k_de*M[30];
...
data{
        ...
        real eta_tr[N_param];
        real  params[57];      // Matrix containing the individual parameters
        real rel_tol;
        real abs_tol;
        real max_num_steps;
...
//priors
sigma1 ~ normal(0,0.01);
//sigma2 ~ normal(0,0.2);
//sigma3 ~ normal(0,0.0001);
//sigma4 ~ normal(0,0.00001);
